// Claimless event handlers (Envio HyperIndex V2 API, envio@^2.0.0).
 ...
 */
IncidentRegistry.IncidentReported.handler(async ({ event, context }) => {
  ...
  const incident: Incident = {
    ...
    // TODO: msg.value (the reporter's bond) is not carried by the event object;
    // capturing event.transaction.value needs field_selection verification for
    // envio 2.32.6. Populated 0 for now.
    stake: 0n, // bond posted by the reporter (not in event)
    reportedAt,
 ...
 */
IncidentRegistry.IncidentChallenged.handler(async ({ event, context }) => {
  const incidentId = event.params.incidentId.toString();
 ...
 */
IncidentRegistry.ChallengeResolved.handler(async ({ event, context }) => {
  const incidentId = event.params.incidentId.toString();
  const incident = await context.Incident.getOrThrow(incidentId);
 ...
 */
IncidentRegistry.IncidentFinalized.handler(async ({ event, context }) => {
  const incidentId = event.params.incidentId.toString();
  const incident = await context.Incident.getOrThrow(incidentId);
 ...
 */
RiskScore.ScoreUpdated.handler(async ({ event, context }) => {
  const agentId = event.params.agentId.toString();
