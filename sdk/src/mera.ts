 ...
 *
 * ⚠️ OUT OF SCOPE (decided 2026-09-21): Claimless does NOT require Mera. The
 * underwriter only needs an ordinary wallet with MON; no contract checks for a
 * passkey. Mera is skipped for this submission and is not on any demo path. The
 * module is kept, correct and tested, in case anyone opts in later.
 *
 * Status: module correct and verified — the derivation is pinned to the
 * authoritative @scure/bip39 by a known-answer test (test/mera.test.ts). A live
 * browser PRF ceremony is NOT part of this submission.
 *
 * REGRESSION NOTE (2026-09-21): the inlined ENGLISH_WORDLIST below was
 * previously corrupt (2035 words, missing 13 from index 16 "acoustic" onward),
 * which produced WRONG addresses that did not match Mera's reference
 * implementation. It is now byte-identical to @scure/bip39 (2048 words).
 */
